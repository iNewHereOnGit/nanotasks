...
const splitTokens = (input) => {
	...
	if (trimmedInput === "") throw new Error(`[ERROR] Empty input, use 'help' for help`);

	const quotesCount = [...trimmedInput.matchAll(quotePattern)].length;

	if (quotesCount % 2 !== 0)
		throw new Error(`[ERROR] Invalid string argument, are you missing a quote? use 'help' for help`);
